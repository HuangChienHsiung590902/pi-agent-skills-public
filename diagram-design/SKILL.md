...
**Input:** the diagram request (type, source such as prose / `.drawio*` / Mermaid, size/detail/audience dials) plus any brand tokens or `.diagram-design` profile marker.

**Output:** one self-contained HTML file with inline SVG/CSS (optional SVG/PNG export only when asked). Report the visual type, semantic pattern if used, size preset, and any fidelity cuts.

## Procedure

Follow the numbered sections already in this file:

1. Run the first-time style-guide gate (§0) before the first diagram in a project.
2. Choose a semantic pattern if behavior is load-bearing, then a visual type (§3) and load only that type reference.
3. Confirm type, size preset, and budget cuts before drawing.
4. Draw from `assets/` templates using §5–§8 primitives; for imports follow §11 (`scripts/drawio_extract.py` / `scripts/mermaid_extract.py`).
5. Run the §9 taste gate and `scripts/self_check.py` before delivering.

## Rules and Limitations

- Resolve relative paths from this Skill directory (`references/`, `assets/`, `scripts/`).
- Load type/semantic/animation references only when selected; do not dump the whole catalog.
- Stay inside the §7 complexity budget unless the import dial is `faithful`.
- Connector rules in §6 are mandatory; diagonal slants and labels sitting on strokes are automatic fails.
- Never invent or silently drop components on an import. Export PNG/SVG only when the user asks.

## Verification

- §9 checklist passes, including `python3 scripts/self_check.py <file>` from this Skill directory.
- Accessible SVG contract holds (`role="img"`, prefixed `<title>` / `<desc>`).
- Output is a single self-contained HTML file that renders without extra assets (Google Fonts excepted).
