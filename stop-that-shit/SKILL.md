...
Use when the user invokes Stop That Shit, or when work is bounded, review-only,
at risk of scope creep, speculative hardening, extra hashing or dependencies,
repeated audit loops, or other unrequested expansion.

## Inputs and Outputs

### Inputs

- The user's requested work and the task mode (`review`, `answer`, `monitor`, or `change`).

### Outputs

- The requested result, necessary consequences, and the evidence that makes the task complete.
- No extra audit loop, disclaimer, or process write-up added only to appear cautious.

## Procedure

1. Follow the Stop Ladder before adding work the user did not name.
2. Respect the task mode: `review` / `answer` / `monitor` stay read-only unless authorized.
3. Keep necessary callers, tests, accessibility, security, compatibility, and migration when reachable evidence requires them.
4. Finish by reporting the requested result and evidence; do not start another review loop.

## Rules and Limitations

- This Skill is advisory without the Guard; it cannot guarantee model behavior.
...
- Do not invent a file list to appear precise.
- Do not claim the Guard blocked an action unless an explicit mode armed it and it returned a host-specific denial.

## Pitfalls

- Turning internal risk controls into user-facing caveats the user did not ask for.
- Repeating searches, tests, or reviews after the requested result already has enough evidence.
- Treating fewer files or lines as the goal instead of the smallest correct result.
- Using inspection commands as if they changed the current task contract.
