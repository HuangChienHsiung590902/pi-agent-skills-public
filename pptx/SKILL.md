...
3. Create and run a JavaScript file using the [`scripts/slideConverter.js`](scripts/slideConverter.js) library to convert HTML slides to PowerPoint and save the presentation
...
4. **Duplicate, reorder, and delete slides using `scripts/reorder.py`**:
...
5. **Extract ALL text using the `scripts/textExtractor.py` script**:
...
6. **Generate replacement text and save the data to a JSON file**
   ...
   - **VALIDATION**: The scripts/textReplacer.py script will validate that all shapes in your replacement JSON exist in the inventory
     - If you reference a non-existent shape, you'll get an error showing available shapes
     - If you reference a non-existent slide, you'll get an error indicating the slide doesn't exist
     - All validation errors are shown at once before the script exits
   - **NOTE**: The scripts/textReplacer.py script uses scripts/textExtractor.py internally to identify ALL text shapes
...
7. **Apply replacements using the `scripts/textReplacer.py` script**
   ```bash
   python scripts/textReplacer.py working.pptx replacement-text.json output.pptx
   ```

   The script will:
   - First extract the inventory of ALL text shapes using functions from scripts/textExtractor.py
...
## When to Use
PowerPoint document toolkit for slide generation, content modification, and presentation analysis. Ideal for: (1) Building new slide decks from scratch, (2) Editing existing presentation content, (3) Managing slide layouts and templates, (4) Inserting notes and annotations, or handling other presentation-related operations
